...
<template>
  ...
  <div class="relative h-10 flex items-center group">
    ...
    <div v-for="(shift, index) in workShifts" :key="index">
      ...
      <!-- Paseki pracy -->
      <div
        class="absolute top-1/2 h-3 rounded-lg shadow-xl translate-y-[50%] "
      ...
      <!-- Punkt startu -->
      <div
        class="absolute top-6 text-white text-xs px-1 py-0.5 rounded shadow-md transition-opacity opacity-0 group-hover:opacity-100"
        :class="[
      ...
      <!-- Punkt końca -->
      <div
        class="absolute top-6 text-xs text-white px-1 py-0.5 rounded shadow-md transition-opacity opacity-0 group-hover:opacity-100"
        :class="[
          colors[index % colors.length],
          (timeToPercent(shift.endTime) - timeToPercent(shift.startTime)) < 11
